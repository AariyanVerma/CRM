...
function RateList({
  ...
  return (
    <section className={`glass-panel p-4 sm:p-5 ${accentBorder}`}>
      ...
      <div className="mb-2 grid grid-cols-3 gap-3 px-3 text-[0.68rem] font-bold uppercase tracking-[0.16em] text-ink-faint sm:px-4 [&>*]:min-w-0 [&>*]:text-center">
        <span>Purity</span>
        <span>DWT</span>
        <span>Gram</span>
      </div>
      ...
      <ul className="flex flex-col gap-2.5">
        {rows.map((row, i) => (
          <li
            key={row.purity}
            className={`animate-row-in group grid grid-cols-3 items-center gap-3 rounded-2xl border bg-white/80 px-3 py-3.5 shadow-glass-sm backdrop-blur-xl transition duration-300 hover:-translate-y-1 hover:scale-[1.02] hover:bg-white hover:shadow-glass-lg sm:px-4 sm:py-4 [&>*]:min-w-0 [&>*]:text-center ${accentBorder}`}
  ...
  return (
    ...
    >
      ...
      <div className="animate-fade-up mt-10 flex justify-center [animation-delay:260ms]">
        <Link
          href="/calculator"
          className="calc-cta group relative inline-flex min-h-[58px] items-center justify-center overflow-hidden rounded-full bg-slate-900 px-10 text-lg font-extrabold text-white focus-visible:outline focus-visible:outline-2 focus-visible:outline-offset-4 focus-visible:outline-slate-400 active:scale-[0.98]"
        >
          <span className="calc-cta-ring pointer-events-none absolute inset-0 rounded-full" aria-hidden />
          <span className="calc-cta-glow pointer-events-none absolute -inset-2 rounded-full" aria-hidden />
          ...
          <span className="relative z-10 flex items-center gap-2.5 tracking-wide">
            <span className="inline-flex h-8 w-8 items-center justify-center rounded-full bg-white/10 transition duration-300 group-hover:bg-white/20 group-hover:scale-110" aria-hidden>
